...
const updateData = (newRow, oldRow) => {
  ...
  const updateOrder = async (orderId, updatedData) => {
    try {
      const response = await axios.put(`http://localhost:3000/api/v1/orders/${orderId}`, updatedData);
...
const flattenOrderData = (orders) => {
  return orders.map(order => ({...order, "consumerName": order.consumer.name, "productName": order.product.name, "productId": order.product.id, "consumerId": order.consumer.id}));
};
